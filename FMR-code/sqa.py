...
def substrate_background_from_one_curve(x,y):
...
def fmr_mesh_dataframe(dataframe,Dtype='fmr'):
    ...
    Y=np.array([float(i)/1e9 for i in dataframe.columns])
    X,Y=np.meshgrid(Y,X)
    Z=dataframe.values;
    fig=plt.figure()
    ...
    ax=fig.gca()
    surf=ax.pcolormesh(Y,X,Z,vmin=Z.min(),vmax=Z.max(),linewidth=0,cmap=cm.coolwarm,shading='auto')
