...
def fit_one_curve(curvex,curvey,width=[400,400]):
	'''	p[0] sysmetrical height
		p[1] resonance field
		p[2] half linewidth
		p[3] asysmetrical height
	...
	while True:
		...
		if len(ginput)==1:
			...
			parameter,fitted_y=fit_guassion_robust(refinedx,refinedy,x0=x0)
			#plt.plot(refinedx,refinedy,'.')
			plt.plot(refinedx,fitted_y,'.',ms=5)
			plt.draw()
			#plt.pause(0.1)
		elif len(ginput)==0:
			break
		elif len(ginput)>3:
			parameter=np.zeros((1,6))[0]
			break
		else:
			print('ginput error')
	plt.clf()
...
def fit_positive_negtive_spinpumping(x,y):
    ...
    if parameter_negtive[0]==0:
        ...
        half_linewidth=abs(parameter_positive[2])
        ...
        half_linewidth=abs(parameter_negtive[2])
        ahe=(parameter_positive[3]+parameter_negtive[3])/2
        hysteresis=0
        ...
        half_linewidth=(abs(parameter_positive[2])+abs(parameter_negtive[2]))/2
        ahe=(parameter_positive[3]+parameter_negtive[3])/2
...
def fit_kittel_robust(fitx,fity):
	'''
	x is H
	y is frequency
	'''
	def fun_fit(p,x,y):
		return fun_cal_y(p,x)-y
	def fun_cal_y(p,x):
		return 1.76*10**10/(2.0*3.1415926)*(x*(x+p[0]))**(0.5)
	x0=0
	res_robust=least_squares(fun_fit,x0,loss='soft_l1',f_scale=0.1,args=(fitx,fity))
	fitted_y=fun_cal_y(res_robust.x,fitx)
	return res_robust.x,fitted_y
...
def substrate_background_from_one_curve(x,y,folder='E:\\document_pc\\Software Working Zone\\python\\trash',filename='test.txt'):
    ...
    while not okornot==1:
        ...
        okornot=eval(input('ok=1;   not=0:  please input the number'))
...
def origin_get_array(pagename,array,position=[0,0],longname_unit_comment=[]):
	'''
	take care , you should use np.array().reshape((m,n)),to specify the array shape as you need
	'''
	origin=win32com.client.Dispatch('Origin.ApplicationSI')
	origin.visible=1
